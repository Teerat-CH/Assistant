...
class Index:
    ...
    def load_index(self, document_file_path="RAG/index_files/documents.pkl", index_file_path="RAG/index_files/index.bin"):
        if os.path.exists(document_file_path) and os.path.exists(index_file_path):
            ...
            raise FileNotFoundError("Document file path or index file path does not exist.")

    def save_index(self, document_file_path="RAG/index_files/documents.pkl", index_file_path="RAG/index_files/index.bin"):
        os.makedirs(os.path.dirname(document_file_path), exist_ok=True)
        faiss.write_index(self.index, index_file_path)
        pickle.dump(self.documents, open(document_file_path, "wb"))
        print(f"Index saved to {index_file_path} and {document_file_path}")
